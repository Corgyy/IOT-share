    ...
    if utime.ticks_diff(current_time, last_firebase_time) >= 1000:
        if distance is not None and 5 <= distance <= 12:
            ...
            if send_to_firebase(data_to_send):
                # Lưu thêm vào bảng accidents
                accident_data = {
                    "latitude": data_to_send["latitude"],
                    "longitude": data_to_send["longitude"],
                ...
                try:
                    response = urequests.request('POST', accident_db_url, json=accident_data)
